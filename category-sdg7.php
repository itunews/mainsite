...
<div class="row">
    <div class="col l8">
         ...
         <h4 style="font-family:avenirbold;font-size:1.2em;color:#00A4E6;padding-right: 5%;">SDG 7: Affordable and Clean Energy</h2>
...
<div class="sectionfirst"> <!--IND: start the first full width featured article section with gray background -->
                         ...
                         <?php $query = new WP_Query( 'showposts=1& cat=105' ); ?><!--IND: start the Wordpress loop, inquire database and retrieve 1 latest post from cat-7 Emerging Trends-->
...
<div class="container ITU_sectionsecond"> <!--IND: use this for the next section after the featured article -->
     <div class="row wow appear-from-bottom element-animated"> <!--IND: add animation effects to this row -->
                 ...
                 <?php $query = new WP_Query( 'offset=1& showposts=1& cat=105' ); ?> <!--IND: start the Wordpress loop, inquire and retrieve 1 post after the 1st post (or post number 2 done with 'offset=1&'), restrict to category-7 by finding the number of category in Wordpress category list in backend (hover over category and see link URI for cat number-->
...
<!--IND: begin new query -->
         ...
         <?php $query = new WP_Query( 'offset=2& showposts=1& cat=105' ); ?> 
...
<?php endif; ?>
    ...
    <div class="container"> <!--IND: TO UPDATE OLD POSTS: copy this entire container with 2 cards, change the offset to refect current number of posts and paste at the end of this page THIS IS DONE TO GAIN THE HIGHEST LEVEL OF ANIMATION CONTROL AND MAKE EVERYTHING FLUID AND COHESIVE-->
     <div class="row wow appear-from-bottom element-animated">
                 ...
                 <?php $query = new WP_Query( 'offset=3& showposts=1& cat=105' ); ?>
<?php if ( $query->have_posts() ) : while ( $query->have_posts() ) : $query->the_post(); ?>
<a href="<?php the_permalink() ?>" title="<?php the_title(); ?>"> 
...
<?php endif; ?>
         ...
         <?php $query = new WP_Query( 'offset=4& showposts=1& cat=105' ); ?>
<?php if ( $query->have_posts() ) : while ( $query->have_posts() ) : $query->the_post(); ?>
<a href="<?php the_permalink() ?>" title="<?php the_title(); ?>">
